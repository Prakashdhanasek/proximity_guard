...
// //   // Bumped v6 -> v7: storage now also holds the per-face folder label, so old
// //   // cached data is ignored and the app re-enrolls automatically (no reinstall).
...
// //   // Distance threshold (L2). Lower = stricter. 0.90 was too tight for
// //   // real-world lighting/angles — genuine faces often land ~0.9–1.2, so some
// //   // drivers never reached a confident match. 1.10 lets all enrolled drivers
// //   // verify reliably while still rejecting strangers (who score ~1.3+).
// //   // Tune: lower toward 1.0 if a stranger ever matches; raise toward 1.15 if a
// //   // genuine driver still struggles.
...
// //               .map<List<double>>((e) => List<double>.from(e as List))
// //               .toList();
// //           _referenceLabels = List<String>.filled(_referenceEmbeddings.length, 'unknown');
// //         }
...
// //       _consecutiveMiss++;
// //       _consecutiveMatch = 0;
// //       if (_consecutiveMiss >= kMissFrames) {
...
// // lib/core/face_auth_engine.dart
// // v5 — Real face recognition via MobileFaceNet TFLite (192D embeddings)
// //
// // ARCHITECTURE:
// // 1. MLKit FaceDetector detects face bounding box from reference photos / live camera
// // 2. Face is cropped from the source image and resized to 112x112
// // 3. MobileFaceNet.tflite converts 112x112 face → 192-dimensional embedding vector
// // 4. L2 Euclidean distance between live embedding and reference embeddings determines identity
// //
// // THRESHOLD: 1.00 (MobileFaceNet intra-class distance: 0.3–0.7; inter-class: 1.0–2.0)
...
// class FaceAuthEngine {
//   final FlutterSecureStorage _storage = const FlutterSecureStorage();
//   static const String _keyEmbedding = 'safe_drive_mobilefacenet_v6';
...
//         _referenceEmbeddings = decoded.map<List<double>>(
//           (e) => List<double>.from(e as List),
//         ).toList();
//         if (_referenceEmbeddings.isNotEmpty) {
//           isEnrolled = true;
//           print('[Auth] MobileFaceNet embeddings loaded from secure storage (${_referenceEmbeddings.length} faces).');
...
//     // Find minimum distance across all enrolled reference faces
//     double minDist = double.infinity;
//     for (final ref in _referenceEmbeddings) {
//       final d = _euclidean(ref, liveEmbedding);
...
//           if (embedding != null) {
//             embeddings.add(embedding);
//             print('[Auth] Enrollment embedding extracted from $fileName (192D)');
//           } else {
//             print('[Auth] Failed to extract embedding from $fileName');
//           }
//         } else {
...
//     try {
//       await _storage.write(key: _keyEmbedding, value: jsonEncode(embeddings));
//       print('[Auth] ${embeddings.length} embeddings saved to secure storage.');
//     } catch (e) {
//       print('[Auth] Storage write failed: $e');
//     }
//   }
...
//       isEnrolled = true;
//       try {
//         await _storage.write(key: _keyEmbedding, value: jsonEncode(_referenceEmbeddings));
