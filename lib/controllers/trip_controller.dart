...
class TripController extends ChangeNotifier {
  ...
  // Alert types that should appear only ONCE per trip (no repeats, even after
  // the on-screen alert is dismissed). "Too close" and "Speed limit" go here.
  final Set<AlertType> _shownOnce = {};
  static const Set<AlertType> _onceOnlyTypes = {
    AlertType.forwardDistance,
  ...
  void startTrip() {
    ...
    _overSpeedCount = 0;
    _totalSamples = 0;
    _shownOnce.clear(); // reset the "show once" tracking for the new trip
  ...
  void _addAlert({
    ...
    // "Too close" and "Speed limit" should appear only ONCE for the whole trip.
    if (_onceOnlyTypes.contains(type)) {
      if (_shownOnce.contains(type)) return; // already shown once — never again
      _shownOnce.add(type);
    }
